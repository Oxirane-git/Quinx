...
def load_existing_emails(leads_dir: str) -> set:
    """Load all emails already saved, to exclude them as duplicates.

    Checks leads.db first (fast indexed query); falls back to scanning
    CSV files in leads_dir if the DB does not exist yet.
    """
    # Primary: query the DB
    try:
        import sys as _sys
        _sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
        from db_handler import get_existing_emails, DB_PATH
        if os.path.exists(DB_PATH):
            seen = get_existing_emails()
            print(f"[INFO] Loaded {len(seen)} existing emails from DB for dedup")
            return seen
    except Exception as e:
        print(f"[WARN] Could not load from DB ({e}), falling back to file scan")

    # Fallback: scan CSV files in leads_dir (original behaviour)
    seen = set()
