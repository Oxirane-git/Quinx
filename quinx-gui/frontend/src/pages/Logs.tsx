...
export default function Logs() {
 ...
 return (
  <div className="h-full flex flex-col animate-in fade-in duration-500 font-sans text-white pb-6">
   <header className="border-b border-divider pb-4 mb-6 flex justify-between items-end">
    ...
    <div className="flex gap-4 font-mono font-bold tracking-wider">
     <button
      ...
      className="bg-gunmetal border border-divider hover:border-matrix hover:bg-matrix/5 text-white hover:text-matrix px-5 py-2.5 text-xs flex gap-2 items-center transition-all disabled:opacity-40 disabled:cursor-not-allowed shadow-[0_0_10px_rgba(0,0,0,0.5)] rounded-lg"
     >
     ...
     <button
      onClick={deleteAll}
      disabled={campaigns.length === 0}
      className="bg-gunmetal border border-red-500/30 hover:border-red-500 hover:bg-red-900/20 text-red-500 px-5 py-2.5 text-xs flex gap-2 items-center transition-all disabled:opacity-40 disabled:cursor-not-allowed shadow-[0_0_10px_rgba(0,0,0,0.5)] rounded-lg"
     >
   ...
   <div className="bg-gunmetal border border-divider flex-1 overflow-auto bento-hover shadow-2xl relative rounded-lg">
     ...
     <tbody className="divide-y divide-divider/50 text-xs border-t-2 border-matrix/50">
      {loading ? (
       ...
       <tr key={c.id} className="hover:bg-obsidian transition-colors group">
        ...
        <td className="p-4">
         <span className={`px-2 py-1 text-[10px] uppercase font-bold tracking-wider border rounded ${c.status === 'completed' ? 'border-matrix/30 text-matrix bg-matrix/5' : 'border-blue-500/30 text-blue-400 bg-blue-500/10'}`}>
        ...
        <td className="p-4">
         {c.has_leads ? (
          <button
           ...
           className="text-gray-400 text-[10px] uppercase font-bold tracking-wider border border-divider bg-obsidian px-2 py-1 hover:border-matrix hover:text-matrix transition-colors flex items-center gap-1 w-max rounded"
          >
        ...
        <td className="p-4 text-right">
         <button
          ...
          className="opacity-0 group-hover:opacity-100 text-gray-600 hover:text-red-500 transition-all p-2 bg-black border border-zinc-800 hover:border-red-500/50 rounded-lg"
