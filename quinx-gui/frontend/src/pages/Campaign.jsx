...
const Campaign = () => {
    ...
    const [selected, setSelected] = useState(null); // filename slug
    ...
    const handleSave = async () => {
        ...
        setMsg(null);
        try {
            const res = await fetch(`${API}/api/campaigns/save`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ filename, ...form }),
            });
    ...
    const handleDelete = async (filename) => {
        ...
        try {
            await fetch(`${API}/api/campaigns/${filename}`, { method: 'DELETE' });
